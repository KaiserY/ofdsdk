...
fn custom_tags_round_trip() {
  ...
  assert_eq!(
    serialized,
    r#"<ofd:CustomTags xmlns:ofd="http://www.ofdspec.org/2016"><ofd:CustomTag><ofd:SchemaLoc>schemas/custom-tags.xsd</ofd:SchemaLoc><ofd:FileLoc>Tags/CustomTags.xml</ofd:FileLoc></ofd:CustomTag></ofd:CustomTags>"#
  );

  let reparsed: CustomTags = serialized.parse().unwrap();
  assert_eq!(reparsed.custom_tag.len(), 1);
  assert_eq!(reparsed.custom_tag[0].name_space, None);
}
