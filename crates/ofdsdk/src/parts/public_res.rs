...
pub struct PublicRes {
  ...
  pub public_res_font_files: Vec<crate::parts::public_res_font_file::PublicResFontFile>,
  pub public_res_media_files: Vec<crate::parts::public_res_media_file::PublicResMediaFile>,
}
impl PublicRes {
  pub(crate) fn new_from_archive<R: std::io::Read + std::io::Seek>(
    ...
    let mut public_res_font_files_paths: Vec<String> = vec![];
      ...
      if let crate::schemas::res::ResContentChoice::Fonts(value) = choice {
        ...
        for value_1 in &value_0.font {
          if let Some(value) = &value_1.font_file {
            public_res_font_files_paths.push(value.clone());
          }
        }
      }
    }
    let mut public_res_font_files = Vec::with_capacity(public_res_font_files_paths.len());
    for child_path in public_res_font_files_paths {
      let loaded = {
        ...
        crate::parts::public_res_font_file::PublicResFontFile::new_from_archive(
          &resolved_path,
          archive,
        )
      };
      match loaded {
        Ok(child) => public_res_font_files.push(child),
        Err(crate::common::SdkError::ZipError(zip::result::ZipError::FileNotFound)) => {}
        Err(err) => return Err(err),
      }
    }
    let mut public_res_media_files_paths: Vec<String> = vec![];
    for choice in &root_element.xml_children {
      if let crate::schemas::res::ResContentChoice::MultiMedias(value) = choice {
        let value_0 = &**value;
        for value_1 in &value_0.multi_media {
          public_res_media_files_paths.push(value_1.media_file.clone());
        }
      }
    }
    let mut public_res_media_files = Vec::with_capacity(public_res_media_files_paths.len());
    for child_path in public_res_media_files_paths {
      let loaded = {
        let child_path = child_path.as_str();
        let resolved_path = {
          let base_path = root_element.base_loc.clone();
        ...
        crate::parts::public_res_media_file::PublicResMediaFile::new_from_archive(
          &resolved_path,
          archive,
        )
      };
      match loaded {
        Ok(child) => public_res_media_files.push(child),
        Err(crate::common::SdkError::ZipError(zip::result::ZipError::FileNotFound)) => {}
        Err(err) => return Err(err),
      }
    }
    Ok(Self {
      ...
      root_element,
      public_res_font_files,
      public_res_media_files,
    })
  }
  pub(crate) fn save_zip<W: std::io::Write + std::io::Seek>(
    ...
    crate::common::save_zip_parts(
      &self.public_res_font_files,
      zip,
      entry_set,
      |child, zip, entry_set| child.save_zip(zip, entry_set),
    )?;
    crate::common::save_zip_parts(
      &self.public_res_media_files,
      zip,
      entry_set,
      |child, zip, entry_set| child.save_zip(zip, entry_set),
    )?;
