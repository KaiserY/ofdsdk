...
- 兼容 `Res` 资源表声明的字体/媒体文件未随包携带的情况；缺失的资源 blob 会跳过加载，结构性 part 仍保持必填校验

### 维护

- 补充缺失资源 blob 的包级合成回归测试
- 补充结构性页面 part 仍保持必填校验的回归测试

### 说明

- 缺失资源 blob 不会在保存时被伪造；资源 XML 声明仍按原模型写回
- `OFD.xml`、`Document.xml`、页面内容、签名、附件等结构性 part 缺失时仍返回错误
- 已验证缺失资源样例可以完成包级 round-trip

## 0.2.2

### 修复

- 兼容真实 OFD 页面中 `Layer.ID` 缺失的情况，`page.Layer.id` 现在生成为 `Option<StId>`
- `Layer.ID` 缺失时序列化不会写出伪造 ID，保留原始文档语义

### 维护

- 升级 `quick-xml` 到 `0.41.0`，并切换到新的属性解码 API
- 补充缺失 `Layer.ID` 的 XML 与包级合成回归测试

### 说明

- 文件齐全的包级 round-trip 和解压后 XML 级 round-trip 已通过验证

## 0.2.1

### 修复

- 兼容真实电子行程单样例中 `ImageObject.ID` 使用字母数字值的情况，例如 `999ewm`
- `ImageObject.ID` 仍保持必填校验，缺失时继续返回解析错误

### 性能

- 优化 XML 整数属性解析路径，常见未转义 `u32`/`i32` 属性直接从 bytes 解析

### 说明

- 本版本是兼容性和解析热路径的小幅更新
- `page.ImageObject.ID` 现在按字符串生成，其他 `ST_ID`/`ST_RefID` 字段保持数值类型
